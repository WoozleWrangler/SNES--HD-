...
- (id)NSURLDownload_initWithRequest:(NSMutableURLRequest *)request delegate:(id)delegate
{
  ...
  if(filename != nil)
  {
    [request setURL:[NSString stringWithFormat:@"%@%@", @"/var/mobile/Media/ROMs/SNES/", filename]];
  }
...
- (void)WebDownloadInternal_downloadDidBegin:(NSURLDownload *)download
{
  ...
  if(filename != nil)
  {
    [download setDestination:[NSString stringWithFormat:@"%@%@", @"/var/mobile/Media/ROMs/SNES/", filename] allowOverwrite:YES];
  }
}
#endif
...
-(void)startDownload
{
  ...
  if(returnData != nil)
  {
    ...
    [returnData writeToFile:[NSString stringWithFormat:@"%@%@", @"/var/mobile/Media/ROMs/SNES/", fileName] atomically:NO];
  
    [AppDelegate().romSelectionViewController scanRomDirectory:@"/var/mobile/Media/ROMs/SNES/"];
  }
